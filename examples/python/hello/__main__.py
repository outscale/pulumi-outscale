...
KEYPAIR_NAME = f"hello-kp-{STACK_NAME}"

KEYPAIR = outscale.Keypair(resource_name="hello", keypair_name=KEYPAIR_NAME)

SECURITY_GROUP = outscale.SecurityGroup(
    resource_name="hello-sg", description="test security group"
)

SECURITY_GROUP_RULE_SSH = outscale.SecurityGroupRule(
    resource_name="hello-sg-ssh",
    flow="Inbound",
    ...
    block_device_mappings=[
        outscale.VmBlockDeviceMappingArgs(
            ...
            bsu=outscale.VmBlockDeviceMappingBsuArgs(
                volume_size=30, volume_type="gp2", delete_on_vm_deletion=True
            ),
        )
    ],
...
PUBLIC_IP_LINK = outscale.PublicIpLink(
    resource_name="helo-ip-link", public_ip_id=PUBLIC_IP.id, vm_id=VM.id
)
