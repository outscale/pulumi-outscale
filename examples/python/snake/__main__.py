...
VM_TYPE = CONFIG.get("vmType") or "tinav7.c1r1p2"
...
NET = outscale.Net(ip_range="10.192.0.0/16", resource_name="snake-net")

SUB = outscale.Subnet(
    net_id=NET.id, ip_range="10.192.0.0/24", resource_name="snake-subpub1"
)

IGW = outscale.InternetService(resource_name="snake-igw")

IGW_LINK = outscale.InternetServiceLink(
    resource_name="snake-igwl", internet_service_id=IGW.id, net_id=NET.id
)

ROUTE_TABLE = outscale.RouteTable(resource_name="snake-rtb", net_id=NET.id)

ROUTE_TABLE_LINK = outscale.RouteTableLink(
    resource_name="snake-rtbl", subnet_id=SUB.id, route_table_id=ROUTE_TABLE.id
)

ROUTE = outscale.Route(
    resource_name="snake-route",
...
SECURITY_GROUP = outscale.SecurityGroup(
    resource_name="snake-sg", description=" allow ssh, http and https", net_id=NET.id
)

SECURITY_GROUP_RULE_HTTP = outscale.SecurityGroupRule(
    resource_name="snake-sgr-http",
    flow="Inbound",
...
SECURITY_GROUP_RULE_SSH = outscale.SecurityGroupRule(
    resource_name="snake-sgr-ssh",
    flow="Inbound",
    security_group_id=SECURITY_GROUP.id,
    from_port_range=22,
    to_port_range=22,
...
KEYPAIR = outscale.Keypair(
    resource_name="snake-kp", public_key=PUBLIC_KEY, keypair_name="snake-kp"
)
...
VM_STARTUP = cloudinit.Config(
    resource_name="hello-cloudinit",
    base64_encode=True,
    gzip=False,
    parts=[
...
PUBLIC_IP_LINK = outscale.PublicIpLink(
    resource_name="snake-lpi", public_ip_id=PUBLIC_IP.id, vm_id=VM.id
)
...
UPDATE_CMD = command.remote.Command(
    "updateCmd",
