...
LDFLAGS_EXTRAS=
LDFLAGS=$(LDFLAGS_PROJ_VERSION) $(LDFLAGS_EXTRAS) $(LDFLAGS_STRIP_SYMBOLS)
...
.make/build_dotnet: .make/generate_dotnet
	dotnet restore sdk/dotnet/Pulumi.Outscale.csproj
...
.make/build_nodejs: .make/generate_nodejs
	cd sdk/nodejs/ && \
		yarn install && \
		yarn run tsc && \
		cp -r ../../README.md ../../LICENSES package.json yarn.lock ./bin/
	@touch $@
...
# `cmd/pulumi-resource-outscale/schema.json` is valid and up to date.
...
test_dotnet:
	./local_tests.sh dotnet

test_go:
	./local_tests.sh go

test_nodejs:
	./local_tests.sh nodejs

test_python:
	./local_tests.sh python
...
.PHONY: test test_dotnet test_go test_nodejs test_python test_yaml test_provider
...
.make/schema: bin/$(CODEGEN) .make/install_plugins
