// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.outscale.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.String;
import java.util.Objects;

@CustomType
public final class GetImageExportTaskOsuExport {
    /**
     * @return The format of the export disk (`qcow2` \| `raw`).
     * 
     */
    private String diskImageFormat;
    /**
     * @return The name of the OOS bucket the OMI is exported to.
     * 
     */
    private String osuBucket;
    /**
     * @return The URL of the manifest file.
     * 
     */
    private String osuManifestUrl;
    /**
     * @return The prefix for the key of the OOS object corresponding to the image.
     * 
     */
    private String osuPrefix;

    private GetImageExportTaskOsuExport() {}
    /**
     * @return The format of the export disk (`qcow2` \| `raw`).
     * 
     */
    public String diskImageFormat() {
        return this.diskImageFormat;
    }
    /**
     * @return The name of the OOS bucket the OMI is exported to.
     * 
     */
    public String osuBucket() {
        return this.osuBucket;
    }
    /**
     * @return The URL of the manifest file.
     * 
     */
    public String osuManifestUrl() {
        return this.osuManifestUrl;
    }
    /**
     * @return The prefix for the key of the OOS object corresponding to the image.
     * 
     */
    public String osuPrefix() {
        return this.osuPrefix;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetImageExportTaskOsuExport defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private String diskImageFormat;
        private String osuBucket;
        private String osuManifestUrl;
        private String osuPrefix;
        public Builder() {}
        public Builder(GetImageExportTaskOsuExport defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.diskImageFormat = defaults.diskImageFormat;
    	      this.osuBucket = defaults.osuBucket;
    	      this.osuManifestUrl = defaults.osuManifestUrl;
    	      this.osuPrefix = defaults.osuPrefix;
        }

        @CustomType.Setter
        public Builder diskImageFormat(String diskImageFormat) {
            this.diskImageFormat = Objects.requireNonNull(diskImageFormat);
            return this;
        }
        @CustomType.Setter
        public Builder osuBucket(String osuBucket) {
            this.osuBucket = Objects.requireNonNull(osuBucket);
            return this;
        }
        @CustomType.Setter
        public Builder osuManifestUrl(String osuManifestUrl) {
            this.osuManifestUrl = Objects.requireNonNull(osuManifestUrl);
            return this;
        }
        @CustomType.Setter
        public Builder osuPrefix(String osuPrefix) {
            this.osuPrefix = Objects.requireNonNull(osuPrefix);
            return this;
        }
        public GetImageExportTaskOsuExport build() {
            final var o = new GetImageExportTaskOsuExport();
            o.diskImageFormat = diskImageFormat;
            o.osuBucket = osuBucket;
            o.osuManifestUrl = osuManifestUrl;
            o.osuPrefix = osuPrefix;
            return o;
        }
    }
}
